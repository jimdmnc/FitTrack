...
public function handleAttendance(Request $request)
{
    ...
    {
        $records = $request->input('records'); // Expecting an array of records
        $current_time = Carbon::now('Asia/Manila');
        ...
        try {
            foreach ($records as $record) {
                ...
                if ($user->member_status === 'expired' || $user->member_status === 'revoked') {
                    continue; // Skip expired or revoked members
                }
    
                // Check for existing attendance
                $attendance = DB::table('attendances')
                    ->where('rfid_uid', $uid)
                    ->whereDate('time_in', $attendance_date)
                    ->orderBy('time_in', 'desc')
                    ->first();
