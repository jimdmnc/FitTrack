    ...
    public function uploadPayment(Request $request)
    {
        ...
        try {
            ...
            if ($request->hasFile('payment_screenshot')) {
                ...
                // === SAVE TO members_payments ===
                $payment = MembersPayment::create([
                    'rfid_uid' => $user->rfid_uid,
                    'amount' => $request->amount,
                    'payment_method' => 'gcash', // ← STILL GCASH (only GCash uploads screenshot)
                    'payment_screenshot' => $path,
                    'status' => 'pending',
                    'payment_date' => now(),
                ]);
    
                // === UPDATE THE LATEST PENDING RENEWAL (GCash only) ===
                $renewal = Renewal::where('rfid_uid', $user->rfid_uid)
                    ->where('status', 'pending')
                    ->where('payment_method', 'gcash')  // ← Only GCash renewals
                    ->latest('created_at')
                    ->first();
