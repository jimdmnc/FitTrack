...
<div id="upgradeMemberModal" class="fixed inset-0 bg-[#1e1e1e] bg-opacity-70 flex justify-center items-center hidden z-50 transition-opacity duration-300 p-4">
    <div class="bg-[#1e1e1e] rounded-xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto transform transition-all duration-300 scale-95 opacity-0" id="upgradeModalContent">
        ...
        <div class="flex justify-between items-center p-4 border-b border-gray-700 sticky top-0 bg-gradient-to-br from-[#2c2c2c] to-[#1e1e1e] z-10">
            ...
            <button onclick="closeUpgradeModal()" class="text-gray-400 hover:text-red-500 hover:bg-red-900 rounded-full p-2 transition">
        ...
        <form id="upgradeForm" action="{{ route('upgrade.membership') }}" method="POST" class="p-6">
            ...
            <!-- EXACT SAME RFID CONTAINER AS REGISTRATION -->
            <div class="rfid-container mb-8">
                <label for="upgrade_rfid_uid" class="block text-gray-200 font-medium mb-2">New RFID Card <span class="text-red-500">*</span></label>
                <div class="relative">
                    <input 
                        id="upgrade_rfid_uid" 
                        name="uid" 
                        type="text"
                        required
                        class="bg-[#3A3A3A] text-gray-200 border-[#2c2c2c] w-full pr-12 py-4 border rounded-lg cursor-default pointer-events-none select-none focus:ring-2 focus:ring-[#ff5722] focus:border-transparent transition-all text-lg font-mono text-center tracking-widest" 
                    ...
                    <div class="absolute inset-y-0 right-3 flex items-center">
                        ...
                        <button 
                            ...
                            class="ml-2 bg-red-600 text-white rounded-full w-6 h-6 flex items-center justify-center hover:bg-red-500 transition-colors hidden"
                        >×</button>
                    </div>
                </div>
            ...
            <div class="bg-purple-900 bg-opacity-20 border border-purple-600 rounded-xl p-5 text-center">
                <p class="text-purple-300 font-medium">Member will be upgraded to RFID access immediately after card is scanned.</p>
            </div>
            ...
            <div class="flex justify-end gap-4 mt-8 pt-6 border-t border-gray-700">
                ...
                <button 
                    type="submit" 
                    id="submitUpgrade" 
                    class="px-8 py-3 bg-gradient-to-r from-[#ff5722] to-orange-600 hover:from-orange-600 hover:to-red-600 text-white font-bold rounded-lg transition shadow-lg flex items-center disabled:opacity-50 disabled:cursor-not-allowed"
                    disabled
                >
                    Complete Upgrade
                </button>
...
<script>
// Reuse your existing functions — just extended for Upgrade Modal
document.addEventListener("DOMContentLoaded", function () {
    let upgradeRfidInterval = null;

    window.openUpgradeModal = function(userId, currentRfid, name) {
    ...
    window.clearUpgradeRfid = function() {
        const uidInput = document.getElementById('upgrade_rfid_uid');
        const uid = uidInput.value;

        if (!uid) {
            updateRfidStatus('error', 'No card to clear', 'upgrade_rfid_status');
            return;
        }
        ...
        .then(data => {
            if (data.success) {
                uidInput.value = '';
                updateRfidStatus('success', 'Card cleared', 'upgrade_rfid_status');
            } else {
                updateRfidStatus('error', data.message || 'Failed to clear', 'upgrade_rfid_status');
            }
            toggleClearButton('upgrade_rfid_uid', 'clearUpgradeRfidBtn');
            document.getElementById('submitUpgrade').disabled = true;
        })
        .catch(() => {
            updateRfidStatus('error', 'Clear failed', 'upgrade_rfid_status');
        });
    };

    function startUpgradeRfidPolling() {
        stopUpgradeRfidPolling();
        upgradeRfidInterval = setInterval(() => {
    ...
    function stopUpgradeRfidPolling() {
        if (upgradeRfidInterval) {
            clearInterval(upgradeRfidInterval);
            upgradeRfidInterval = null;
        }
    }

    // Reuse your existing toggleClearButton
    window.toggleClearButton = function(inputId, btnId) {
    ...
    // Reuse your existing updateRfidStatus (with target support)
    window.updateRfidStatus = function(type, message, targetId = 'rfid_status') {
        ...
        el.className = `mt-2 text-sm ${colors[type]} flex items-center font-medium`;
    };
});
</script>
@endsection